<template>
  <div class="backup-codes-display">
    ...
    <div class="close-action">
      <n-button type="primary" @click="$emit('close')"> 我已安全保存 </n-button>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const copyAllCodes = async () => {
    try {
      const codesText = props.backupCodes
        .map((code, index) => `${String(index + 1).padStart(2, '0')}. ${code}`)
        .join('\n');
  ...
  const downloadCodes = () => {
    ...
    const content = [
      ...
      '',
      '备用恢复码:',
      ...props.backupCodes.map((code, index) => `${String(index + 1).padStart(2, '0')}. ${code}`),
    ].join('\n');
